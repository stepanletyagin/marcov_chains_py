...
def create_freq_matrix(series):
    ...
    series_freq_array = np.zeros((len(idx) - 1, 1)).astype(np.object)  # array of states for all series

    freq_matrix_len = y_points * x_points
                ...
                for j in range(0, y_points):
                    if (data1[k] > x_min_temp) and (data1[k] < x_max_temp) and (data2[k] > y_min_temp) and (
                        ...
                        if k == 0:  # Разобраться с первым элементом серии
                            seq_idx_cur = i * x_points + j  # Sequence num of elem in matrix (x_points - num of col)
                        else:
                            seq_idx_last = seq_idx_cur
                            seq_idx_cur = i * x_points + j
